...
public final class Bootstraps {
    ...
    /**
     * Copy an array of {@link MemorySegment} addresses into a newly allocated native segment.
     * The resulting segment has size {@code array.length * ADDRESS.byteSize()} and contains
     * one {@link ValueLayout#ADDRESS ADDRESS}-sized slot per element, each holding the address
     * of the corresponding array element.
     * <p>
     * This method is called from generated bytecode for {@code MemorySegment[]} parameters
     * annotated with {@link In @In}.
     *
     * @param allocator the segment allocator to use (must not be {@code null})
     * @param array the source array of memory segments (must not be {@code null}; elements must be native segments)
     * @return a native segment containing the addresses of all array elements (not {@code null})
     */
    public static MemorySegment copySegmentArrayToNative(SegmentAllocator allocator, MemorySegment[] array) {
        MemorySegment segment = allocator.allocate(ValueLayout.ADDRESS, array.length);
        for (int i = 0; i < array.length; i++) {
            segment.setAtIndex(ValueLayout.ADDRESS, i, array[i]);
        }
        return segment;
    }

    /**
     * Copy addresses from a native segment back into a {@link MemorySegment} array.
     * Each {@link ValueLayout#ADDRESS ADDRESS}-sized slot in the native segment is read
     * and stored as a zero-length {@link MemorySegment} in the corresponding array position.
     * <p>
     * This method is called from generated bytecode for {@code MemorySegment[]} parameters
     * annotated with {@link Out @Out}.
     *
     * @param segment the native segment containing addresses to read (must not be {@code null})
     * @param array the destination array to populate (must not be {@code null})
     */
    public static void copySegmentArrayFromNative(MemorySegment segment, MemorySegment[] array) {
        for (int i = 0; i < array.length; i++) {
